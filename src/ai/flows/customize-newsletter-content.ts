...
import { workoutPunGreetingFlow } from '@/ai/flows/workout-pun-generator'; // Import the actual flow
...
const CustomizeNewsletterOutputSchema = z.object({
  greeting: z.string().describe('A friendly greeting with a running-related pun, generated by the workoutPunGreetingFlow tool.'),
...
const prompt = ai.definePrompt({
  ...
  tools: [
    workoutPunGreetingFlow, // Use the imported flow as a tool
  ...
  Tasks to perform:
  1. Generate a personalized greeting: Use the 'workoutPunGreetingFlow' tool. The tool expects an input object like '{ "userName": "string" }'. The user's name is {{{userName}}}. The tool will return an object like '{ "greeting": "string" }'. Use the value of the returned 'greeting' field for the 'greeting' field in your final JSON output. Ensure this greeting is fun and motivational, with a running-related pun.
...
const customizeNewsletterFlow = ai.defineFlow(
  ...
  async (input: CustomizeNewsletterInput): Promise<CustomizeNewsletterOutput> => {
    ...
    if (!output) {
      ...
      // Try to generate a greeting even if the main prompt fails, using the tool directly as a fallback
      let fallbackGreeting = `Hello ${input.userName}, your personalized newsletter could not be generated by the AI at this time.`;
      try {
        const greetingResult = await workoutPunGreetingFlow({ userName: input.userName });
        if (greetingResult && greetingResult.greeting) {
          fallbackGreeting = greetingResult.greeting;
        }
      } catch (greetingError) {
        console.error("[customizeNewsletterFlow] Fallback greeting generation also failed:", greetingError);
      }
        ...
        output.topStories = output.topStories.filter(story =>
            ...
            (() => {
                if (story.url && !/^https?:\/\//i.test(story.url)) {
                  // Ensure we're modifying the actual object in the array
    ...
    if ((newsToolResults.error || !newsToolResults.articles || newsToolResults.articles.length === 0) && output.topStories.length > 0) {
      console.warn("[customizeNewsletterFlow] News tool returned no articles or an error, but AI generated topStories. Overriding to empty array.");
      output.topStories = [];
    }
